...
Ubody = Dop(qp2al(0, -0.9)) * Sop(-log(1.15));
Ustem = Dop(qp2al(0, 2.8)) * Sop(log(2.0));
Uleaf = Dop(qp2al(2.1, 2.55)) * Rop(-55*pi/180) * Sop(log(1.9));

objs = { ...
    0, Ubody, 0.17; 1, Ubody, 0.21; 2, Ubody, 0.21; 3, Ubody, 0.21; ...
    0, Ustem, 0.05; ...
    0, Uleaf, 0.12};

gBody = unitaryGate(1:N, Ubody);
gStem = unitaryGate(1:N, Ustem);
gLeaf = unitaryGate(1:N, Uleaf);
gateOf = containers.Map({'b','s','l'}, {gBody, gStem, gLeaf});
objGateKey = {'b','b','b','b','s','l'};

nSet = 3^N;
nPauli = 4^N;
sig = {eye(2), [0 1;1 0], [0 -1i;1i 0], [1 0;0 -1]};

Hs = cell(numel(objGateKey), 1);
...
for o = 1:numel(objGateKey)
    ...
    for setIdx = 0:nSet-1
        ...
        gates = [prep(:); {gateOf(objGateKey{o})}; basis(:)];
        c = quantumCircuit(vertcat(gates{:}), N);
    ...
    Hs{o} = max(real(sum((conj(Cmat) * rho) .* Cmat, 2)), 0);
    fprintf('object %d/%d done\n', o, numel(objGateKey));
end

H = zeros(res*res, 1);
for o = 1:numel(objGateKey), H = H + objs{o,3} * Hs{o}; end
H = reshape(H, res, res);
...
imwrite(up, fullfile('output', 'apple_qcg.png'));
fprintf('wrote output/apple_qcg.png\n');
